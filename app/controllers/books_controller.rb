class BooksController < ApplicationController
  ...
  def update
    @user = current_user
    old_book = @user.lists.find(params[:lists][:old_list_id]).books.find_by(api_id: params[:lists][:book_id])
    old_book.destroy
    new_book = @user.lists.find(params[:new_list_id]).books.create(old_book.attributes)
